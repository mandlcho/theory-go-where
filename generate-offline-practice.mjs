  ...
  <style>
    ...
    *{box-sizing:border-box} html{scroll-behavior:smooth}
    ...
    @media(max-width:600px){
      .topbar{position:sticky;top:0;padding:calc(11px + env(safe-area-inset-top)) max(12px,env(safe-area-inset-right)) 11px max(12px,env(safe-area-inset-left))}.topbar-inner{gap:10px}.brand{gap:9px}.brand-mark{width:38px;height:38px;border-radius:10px}.brand h1{font-size:.98rem}.brand p{display:none}.ghost-light{min-height:38px;padding:0 10px;font-size:.78rem;white-space:nowrap}
...
<body data-view="home">
  ...
  <script>
    ...
    function show(view) { homeView.hidden=view!=="home"; examView.hidden=view!=="exam"; resultView.hidden=view!=="result"; document.body.dataset.view=view; $("#homeButton").hidden=view==="home"; setPaletteOpen(false); window.scrollTo({top:0,behavior:"smooth"}); }
    ...
    document.addEventListener("keydown",(event)=>{if(event.key==="Escape"&&questionSidebar.classList.contains("mobile-open")){setPaletteOpen(false,true);return;}if(examView.hidden||event.target.matches("input"))return;if(event.key==="ArrowLeft")$("#prevButton").click();if(event.key==="ArrowRight"&&!$("#nextButton").hidden)$("#nextButton").click();});
