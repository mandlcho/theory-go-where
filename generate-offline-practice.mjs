...
<body data-view="home">
  ...
  <script>
    ...
    const REVIEWED_EXPLANATIONS = {
      "6:29": {
        reason:"When reversing, the car travels backwards, so look through the rear windscreen to watch the path you are entering for people and obstacles. Also check around the car periodically, including the front as it swings out when you turn.",
        wrong:{A:"Watching the kerb and side helps you judge clearance, but misses the path directly behind the car. You could reverse into someone or something there.",B:"Looking ahead and at the side leaves the area behind the car unobserved. That is where you are moving, so you need to look backwards through the rear windscreen."}
      },
      "5:15": {
        reason:"Revocation cancels the licence; it does not automatically become valid again after a waiting period. Under the revocation rule tested here, you must wait one year from the revocation date before retaking the required theory and practical tests to obtain a new licence.",
        wrong:{B:"Two years is longer than the one-year waiting period for retaking the tests under this rule. Waiting alone does not restore the licence.",C:"Three years is not the waiting period under this rule. You can retake the required tests after one year, but must pass them to obtain a new licence."}
      },
      "6:16": {
        reason:"A newly qualified driver's probation lasts 12 months from the date the licence is issued. During that first year, the driver must display probation plates and is subject to the probationary-driver rules. This is a probation period, not the expiry date of the licence.",
        wrong:{B:"Two years doubles the actual probation period. A new driver's probation lasts the first 12 months after licence issue.",C:"Three years would mean 36 months of probation. The required probation period is only 12 months from licence issue."}
      },
      "4:24": {
    ...
    function officialSource(q) {
      if(Number(q.paper)===5&&q.number===15)return {title:"Traffic Police — Basic Theory handbook: licence revocation",url:"https://www.police.gov.sg/-/media/SPF/Advisories/TP/BT-ENG-2126.pdf"};
      const s=(q.question+" "+q.options.map(o=>o.text).join(" ")).toLowerCase();
      if(/demerit|probation|driving test|licen[cs]e|suspend|revok/.test(s)) return {title:"Traffic Police — Driver Improvement Point System",url:"https://www.police.gov.sg/Knowledge-Hub/Traffic/Traffic-Matters/Driver-Improvement-Point-Systems"};
